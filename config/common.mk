...
# Set CM_BUILDTYPE
ifdef CMB_NIGHTLY
    CMB_BUILDTYPE := NIGHTLY
endif
ifdef CMB_EXPERIMENTAL
    CMB_BUILDTYPE := EXPERIMENTAL
endif
ifdef CMB_RELEASE
    CMB_BUILDTYPE := RELEASE
endif

ifdef CMB_BUILDTYPE
    ifdef CMB_EXTRAVERSION
        # Force build type to EXPERIMENTAL
        CMB_BUILDTYPE := EXPERIMENTAL
        # Remove leading dash from CM_EXTRAVERSION
        CMB_EXTRAVERSION := $(shell echo $(CMB_EXTRAVERSION) | sed 's/-//')
        # Add leading dash to CM_EXTRAVERSION
        CMB_EXTRAVERSION := -$(CMB_EXTRAVERSION)
    endif
else
...
ifdef CMB_RELEASE
    CMB_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CMB_BUILD)
else
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        CMB_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(CMB_BUILDTYPE)-$(CMB_BUILD)$(CMB_EXTRAVERSION)
    else
        CMB_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(CMB_BUILDTYPE)-$(CMB_BUILD)$(CMB_EXTRAVERSION)
    endif
endif

PRODUCT_PROPERTY_OVERRIDES += \
  ro.cmb.version=$(CMB_VERSION) \
  ro.modversion=$(CMB_VERSION)
